...
class Name(Field):
    def __init__(self, value):
        try:
            ...
            raise InvalidNameError("Invalid name. Please enter a name containing only letters.")
...
def birthdays(book):
    if not any(record.birthday for record in book.data.values()):
        print("No birthdays information available.")
    else:
